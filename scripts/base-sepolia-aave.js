...
async function runAaveFlow(signer, status) {
  ...
  const minGasEth = process.env.BASE_SEPOLIA_AAVE_MIN_ETH || "0.001";
  ...
  if (status.ethBal < minGasWei) {
    console.log(
      `\nNeed at least ${minGasEth} native ETH for gas (have ${ethers.formatEther(status.ethBal)}). Fund: https://www.alchemy.com/faucets/base-sepolia`
    );
